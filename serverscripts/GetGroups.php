...
if (validateInputData(array('SessionId', 'ItemsOnPage', 'PageNumber'))) {
	if (validateSession(null) && connectToDatabase()) {
		...
		$result1         = mysql_query('SELECT COUNT(*) AS Records FROM Groups');
		...
		$result2 = mysql_query("SELECT * From Groups LIMIT $startRecord, $itemsOnPage");
		
		if ($result2 && mysql_num_rows($result2) > 0) {
			while ($row2 = mysql_fetch_array($result2)) {
				$id   = $row2['Id'];
				$group = safeFromSql($row2['Name']);
